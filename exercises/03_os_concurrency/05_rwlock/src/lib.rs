...
impl<T> RwLock<T> {
    ...
    pub fn read(&self) -> RwLockReadGuard<'_, T> {
        loop {
            ...
            if (prev_state & WRITER_HOLDING) != 0
                || (prev_state & WRITER_WAITING) != 0
                || (prev_state & READER_MASK) == READER_MASK
            {
                core::hint::spin_loop();
                continue;
            }
    ...
    pub fn write(&self) -> RwLockWriteGuard<'_, T> {
        ...
        loop {
            let prev_state = self.state.load(Ordering::Relaxed);
            if (prev_state & WRITER_HOLDING) != 0 || (prev_state & READER_MASK) != 0 {
                core::hint::spin_loop();
                continue;
            }
            match self.state.compare_exchange(
