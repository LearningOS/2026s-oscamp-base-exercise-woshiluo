...
impl Tlb {
    ...
    pub fn insert(&mut self, vpn: u64, ppn: u64, asid: u16, flags: u64) {
        ...
        self.fifo_ptr += 1;
        if self.fifo_ptr >= self.capacity {
            self.fifo_ptr = 0;
        }
    }
    ...
    pub fn valid_count(&self) -> usize {
        ...
        self.entries.iter().filter(|x| x.valid).count()
    }
}
        ...
        match self.tlb.lookup(vpn, self.current_asid) {
            ...
            _ => {
                match self
                    .page_table
                    .iter()
                    .find(|x| x.0 == self.current_asid && x.1.vpn == vpn)
                    .map(|x| &x.1)
                {
                    Some(x) => {
                        self.tlb.insert(vpn, x.ppn, self.current_asid, x.flags);
                        Some(x.ppn)
                    }
                    _ => None,
                }
