...
pub fn double_in_thread(numbers: Vec<i32>) -> Vec<i32> {
    std::thread::spawn(move || numbers.iter().map(|x| x * 2).collect::<Vec<i32>>())
        .join()
        .unwrap()
}
...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    let handle_a = std::thread::spawn(move || a.iter().sum());
    let handle_b = std::thread::spawn(move || b.iter().sum());
...
pub fn named_sleeper(value: i32, ms: u64) -> i32 {
    ...
    builder
        .spawn(move || thread::sleep(Duration::from_millis(ms)))
        .unwrap()
        .join()
        .unwrap();
    value
...
pub fn scoped_slice_sum(a: &[i32], b: &[i32]) -> (i32, i32) {
    ...
    thread::scope(|s| {
        ...
        let handle_a: ScopedJoinHandle<'_, i32> = s.spawn(|| a.iter().sum());
        let handle_b: ScopedJoinHandle<'_, i32> = s.spawn(|| b.iter().sum());
        (handle_a.join().unwrap(), handle_b.join().unwrap())
    })
}
